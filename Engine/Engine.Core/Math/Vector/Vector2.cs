...
public struct Vector2 : IEquatable<Vector2>
{
    ...
    {
        var scale = Mathf.InverseSqrtFast(X * X + Y * Y);
        X *= scale;
        Y *= scale;
    }
    ...
    {
        var scale = Mathf.InverseSqrtFast(vec.X * vec.X + vec.Y * vec.Y);
        vec.X *= scale;
        vec.Y *= scale;
        return vec;
    }
    ...
    [Pure]
    public static Vector2 Transform(Vector2 vec, Quaternion.Quaternion quat)
    {
    ...
    public static void Transform(in Vector2 vec, in Quaternion.Quaternion quat, out Vector2 result)
    {
        var v = new Quaternion.Quaternion(vec.X, vec.Y, 0, 0);
    ...
    [Pure]
    public static Vector2 operator *(Quaternion.Quaternion quat, Vector2 vec)
    {
        Transform(in vec, in quat, out var result);
        return result;
    }
