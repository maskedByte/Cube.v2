...
public struct Vector4 : IEquatable<Vector4>
{
        ...
        set
        {
            switch (index)
            {
                ...
                case 3:
                    ...
                    break;
                default:
                    throw new IndexOutOfRangeException($"You tried to set this {nameof(Vector4)} at index: " + index);
            }
        }
    }
    ...
    {
        var scale = Mathf.InverseSqrtFast(X * X + Y * Y + Z * Z + W * W);
        X *= scale;
        Y *= scale;
        Z *= scale;
        W *= scale;
    ...
    {
        var scale = Mathf.InverseSqrtFast(vec.X * vec.X + vec.Y * vec.Y + vec.Z * vec.Z + vec.W * vec.W);
        vec.X *= scale;
        vec.Y *= scale;
        vec.Z *= scale;
        vec.W *= scale;
    ...
    [Pure]
    public static Vector4 Transform(Vector4 vec, Quaternion.Quaternion quat)
    {
    ...
    public static void Transform(in Vector4 vec, in Quaternion.Quaternion quat, out Vector4 result)
    {
        var v = new Quaternion.Quaternion(vec.X, vec.Y, vec.Z, vec.W);
        Quaternion.Quaternion.Invert(in quat, out var i);
        Quaternion.Quaternion.Multiply(in quat, in v, out var t);
        Quaternion.Quaternion.Multiply(in t, in i, out v);
    ...
    /// <returns>The transformed vector.</returns>
    [Pure]
    public static Vector4 operator *(Quaternion.Quaternion quat, Vector4 vec)
    {
        Transform(in vec, in quat, out var result);
        return result;
    }
