...
public struct Vector3 : IEquatable<Vector3>
{
    ...
    {
        var scale = Mathf.InverseSqrtFast(X * X + Y * Y + Z * Z);
        X *= scale;
        Y *= scale;
        Z *= scale;
    }
    ...
    {
        var scale = Mathf.InverseSqrtFast(vec.X * vec.X + vec.Y * vec.Y + vec.Z * vec.Z);
        vec.X *= scale;
        vec.Y *= scale;
        vec.Z *= scale;
        return vec;
    ...
    [Pure]
    public static Vector3 Transform(Vector3 vec, Quaternion.Quaternion quat)
    {
    ...
    public static void Transform(in Vector3 vec, in Quaternion.Quaternion quat, out Vector3 result)
    {
    ...
    public static void CalculateAngle(in Vector3 first, in Vector3 second, out float result)
    {
        ...
        result = MathF.Acos(Mathf.Clamp(temp / (first.Length * second.Length), -1.0f, 1.0f));
    }
    ...
    [Pure]
    public static Vector3 operator *(Quaternion.Quaternion quat, Vector3 vec)
    {
        Transform(in vec, in quat, out var result);
        return result;
    }
