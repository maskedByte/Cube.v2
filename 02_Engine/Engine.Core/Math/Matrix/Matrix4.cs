...
public struct Matrix4 : IEquatable<Matrix4>
{
    ...
    [Pure]
    public Quaternion.Quaternion ExtractRotation(bool rowNormalize = true)
    {
        var row0 = Row0.Xyz;
        var row1 = Row1.Xyz;
        var row2 = Row2.Xyz;
        ...
        var q = default(Quaternion.Quaternion);
        var trace = 0.25 * (row0[0] + row1[1] + row2[2] + 1.0);

        if (trace > 0)
        {
            var sq = Mathf.Sqrt(trace);
        ...
        {
            var sq = 2.0 * Mathf.Sqrt(1.0 + row0[0] - row1[1] - row2[2]);
        ...
        {
            var sq = 2.0 * Mathf.Sqrt(1.0 + row1[1] - row0[0] - row2[2]);
        ...
        {
            var sq = 2.0 * Mathf.Sqrt(1.0 + row2[2] - row0[0] - row1[1]);
    ...
    /// <param name="result">A matrix instance.</param>
    public static void CreateFromQuaternion(in Quaternion.Quaternion q, out Matrix4 result)
    {
    ...
    /// <returns>A matrix instance.</returns>
    [Pure]
    public static Matrix4 CreateFromQuaternion(Quaternion.Quaternion q)
    {
    ...
    public static Matrix4 CreatePerspectiveFov(float fov, float width, float height, float zNear, float zFar)
    {
        ...
        var h = Mathf.Cos(0.5f * rad) / Mathf.Sin(0.5f * rad);
